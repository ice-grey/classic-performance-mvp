...
import { PenTool, Heart, Ticket, Users, Asterisk } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ activeView, setView }) => {
  const links = [
    { id: ViewState.CURATE, label: '큐레이션', icon: PenTool },
    { id: ViewState.PERFORMANCES, label: '공연 검색', icon: Ticket },
    { id: ViewState.MY_PICK, label: 'MY PICK', icon: Heart },
        ...
        <div className="flex items-center space-x-1">
          {links.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setView(id)}
              className={`flex items-center space-x-3 px-5 py-2 transition-all duration-300 text-xs font-bold tracking-tight
                ${activeView === id
                  ? 'bg-stone-900 text-white shadow-xl'
                  : 'text-stone-400 hover:text-stone-900 hover:bg-stone-50'}`}
            >
              <Icon className={`w-4 h-4 ${id === ViewState.MY_PICK && activeView === id ? 'fill-white' : ''}`} />
              <span className="hidden lg:inline">{label}</span>
            </button>
          ))}
        </div>

        <div className="hidden md:flex items-center space-x-3 text-[10px] font-black text-stone-300 uppercase tracking-widest border-l border-stone-100 pl-6">
          <span>Powered by Kyth. 2025</span>
        </div>
      </div>
    </nav>
  );
